    ...
    function select(fields, limit, offset) {
      ...
      this.joinList.forEach((val) => {
        ...
        str =
          str +
          joinOnStr +
          (val.whereList.length > 0
            ? ` ${val.whereList[0][3]}${joinWhereData.whereStr}`
            : "");
    ...
    let colStr = columns
      .map((col) => {
        ...
        return [
          ...
          col.autoIncrement && col.primaryKey && primaryKeys.length === 1
            ? "AUTOINCREMENT"
            : null,
